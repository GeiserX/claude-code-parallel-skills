...
> **Depends on [oh-my-claudecode](https://github.com/anthropics/oh-my-claudecode)** for persistence
> (`ralph` / `cancel`); the audit fan-out uses the Agent tool. Install OMC first (see the repo README).
...
flowchart TB
  ...
  F --> G["4d VERIFY — fresh reviewer vs file:line + mermaid-lint + secret scan"]
...
- Everything else is the **scope**: explicit repo names ("repo-a, repo-b"), a workspace phrase ("all active
  repos", "all of <workspace>"), a path, or **empty → the current repo only**. This is the outer-loop
  domain.

## Step 1 — build the CANONICAL worklist → write the LEDGER

**Never enumerate with a raw `ls`.** A workspace root often holds many dirs that are **worktrees**
(`*-wt*`, `*.wt-*`, `wt-*`) and duplicate checkouts, not distinct repos. Build the list deliberately:

1. **Resolve names → canonical checkouts.** For a multi-repo scope, read your **repo map** at runtime for
   the authoritative repo↔status list — e.g. a top-level `CLAUDE.md`/`REPOS.md` you maintain, or
   `gh repo list <org>`. Do **not** hardcode it (it drifts), and never copy any credential-bearing content
   from it. Map each repo to its single canonical checkout (`<repos-root>/<repo>`).
2. **Collapse worktrees.** A dir whose `git rev-parse --git-common-dir` points *outside itself* is a
   worktree — fold it into its canonical repo; never treat 8 `<repo>.wt-*` dirs as 8 repos.
3. **Classify + filter.** Include **Active** repos by default. **Exclude ARCHIVED** repos
   (`gh repo view <org>/<repo> --json isArchived` → true — pushes 403; their changes go to a live sibling)
   and **Reference/personal** repos unless the scope explicitly names them. A repo with no local clone →
   record `BLOCKED (not cloned)`, don't drop it silently.
...
edits go into each repo): a `.docs-loop/` dir at your workspace root for a multi-repo scope, else the
scratchpad run dir. Three files: `DOCS-LOOP-LEDGER.md`, append-only `DOCS-LOOP-WORKLOG.md` (newest at
bottom, evidence on every done), `DOCS-LOOP-DEFERRED.md`.
...
- If OMC is available, invoke the **`ralph`** skill with the goal "bring docs up to date across `{{SCOPE}}`,
  ...
  turn — **not a feeling**.
- **Clean exit is mandatory:** on completion or a real blocker, invoke **`cancel`** and clear any
  persistence flag. A dangling persistence state re-fires the Stop hook forever.
...
1. **Canonical worklist** (Step 1) — never a raw `ls`; collapse worktrees; denylist archived repos.
2. **Work in a throwaway `origin/main` worktree — NEVER the live checkout.** `git fetch origin`, then
   ...
   active-autopilot-race problem *by construction* (a checkout may carry dozens of uncommitted files on a
   feature branch). Read code truth and make edits **there**; remove the worktree when done. Never
...
the file.** Preserve headings, structure, and the doc's existing voice. Follow good doc hygiene: **docs
state intent, not sources** (never name a person or cite an internal call/chat — state the decision as
"we"); Mermaid in GitHub-bound `.md`, used where it helps. Respect a **diff-size cap** — a "doc update" that
rewrites a large fraction of a file is a red flag; stop and re-scope.
...
- **Lint every touched Mermaid diagram** with a mermaid linter/CLI (e.g. `@mermaid-js/mermaid-cli`'s `mmdc`,
  or any mermaid parser) — it must parse cleanly for all blocks. A broken diagram renders as an error box on
  GitHub.
...
**zero AI attribution** (no `Co-Authored-By` / "Generated with Claude"). Unset any ambient GitHub token env
var before a github.com op if your setup needs it. **Open a PR — never push to `main`, never force-push,
never `--no-verify`.** After the PR, `gh run watch`; red CI on a docs-only change (usually a broken diagram)
→ fix or revert. Capture the **PR link** into the ledger row + worklog entry — this is the deliverable a
human reads: *docs → push → link, always.* Then remove the throwaway worktree.

**4f. Mark the repo DONE.** Set the ledger row `Status → DONE`, fill `Docs touched` + `link`, append the
worklog entry (evidence: the file:line anchors, `mermaid lint OK n/n`, the PR link). Update `plateau_repos`:
...
**Defer a single doc** (park in `DOCS-LOOP-DEFERRED.md`: Context / Default-taken / To-change; if you maintain
a knowledge base, **consult it first** to settle intent) when the change needs judgment the code can't give:
a **roadmap/strategy/positioning** call (what we *intend*, not what the code *is*), an **ambiguous** stale
section where you'd be guessing, a **product decision** (deprecate/rename a public thing),
**licensing/legal/security-posture** wording, deleting a doc, or **anything that would name a person or cite
an internal source**. Ship the confident edits, note the deferred doc, still mark the repo DONE — set the row
`DEFERRED` only if the *whole* repo is blocked on it.

**Clean exit (mandatory):** on any halt, invoke `cancel`, clear persistence, and report — per repo: what
shipped (PR link + evidence), what was deferred, what was skipped and why, and the θ to lower to go deeper.
...
- **PR-only, no AI attribution, keep CI green.** Conventional `docs:` commits; `gh run watch`; never
  force-push / `--no-verify` / direct-to-main; never change global git config.
- **Docs state intent, not sources** — never name a person or cite an internal call/chat; state the decision
  as "we". Easy to read, build from the familiar; Mermaid where it helps and **always lint-clean**.
...
- **Defer, don't ask.** Go uninterrupted; for a would-be question consult the knowledge base (or `/research!`)
  first, else pick the reversible default (annotate not rewrite; draft PR not merge), park it in
  `DOCS-LOOP-DEFERRED.md`, and keep moving. Surface to the user ONLY a roadblock that blocks all progress.
